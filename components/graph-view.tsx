...
export function GraphView(props: GraphViewProps) {
  ...
  return (
    <div
      ref={ref}
      className="not-prose relative w-full max-w-full h-[min(600px,70vh)] rounded-xl border overflow-hidden bg-fd-background"
    >
      {mount && size.width > 0 && (
        <ClientOnly {...props} containerRef={ref} size={size} />
      )}
    </div>
  );
}

function fitGraph(graph: ForceGraphMethods<Node, Link> | undefined) {
...
function ClientOnly({
  ...
  useEffect(() => {
    ...
    const id = window.setTimeout(() => fitGraph(graphRef.current), 500);
    return () => window.clearTimeout(id);
  }, [size.width, size.height, enrichedNodes]);
  ...
  return (
    <>
      <ForceGraph2D<NodeType, LinkType>
        ...
        ref={{
          ...
          set current(fg) {
            ...
            if (fg) {
              ...
              fg.d3Force('collision', forceCollide(48));
              fitGraph(fg);
            }
          },
        }}
        ...
        onEngineStop={() => fitGraph(graphRef.current)}
        linkWidth={2}
        enableNodeDrag
