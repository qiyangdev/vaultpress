...
- `public/` — Generated static assets (vault media, Canvas files, and canvas-referenced assets)
...
- `.canvas` files under `GENERATE_INCLUDE` are copied to `public/`
- Media referenced by canvas nodes are copied to `public/` even when outside `GENERATE_INCLUDE`
- An MDX page is generated under `content/` for each canvas file
...
1. Put `.canvas` files in your vault (for example `canvas/demo.canvas`)
2. Include the canvas folder in `GENERATE_INCLUDE`
3. Run `pnpm generate` — files sync to `public/`, and pages are generated under `content/`
4. Open the canvas on the site (for example [/canvas/demo](/canvas/demo))

Supported node types: text, file, link, and group — with edges, colors, group backgrounds, media files, and MDX preview for Markdown notes. Drag to pan; scroll to zoom.
